// ACT-Omega v27.0: Task 28 Fast-Trip Field Expedition Logger
...
let totalTensorsComputed = 320000;
let superStepEpoch = 5040000;
let totalB2Events = 2700;
let totalB3Events = 210;
...
console.log("\x1b[1m\x1b[32m [ACT-OMEGA V27.0]: TASK 28 DRAIN-WATCHDOG LOGGER ACTIVE\x1b[0m");
...
function flushOfflineQueue(ws: WebSocket) {
  ...
  console.log(` \x1b[32m[SYNC COMPLETE]\x1b[0m: All buffered frames committed. Resumed live lockstep.`);
}
...
function initConnection(targetUrl: string, mode: "USB_TETHER" | "WIFI_UNTETHERED") {
  ...
  ws.onopen = () => {
    ...
    console.log(` \x1b[32m[CONNECTED]\x1b[0m: Transport latched -> ${modeTag} @ 15.965 Hz`);
  ...
  ws.onclose = () => {
    if (activeSocket === ws) {
      ...
      if (mode === "USB_TETHER") {
        ...
        console.log(" \x1b[33m[EXPEDITION OFFLINE]\x1b[0m: Node untethered in field. Persisting to local ledger...");
      }
    }
  };
  ws.onerror = () => { ws.close(); };
...
// Reconnection probe (every 2.5 seconds)
setInterval(() => {
...
setInterval(() => {
  ...
  // 4. FAST-TRIP DRAIN WATCHDOG: Detect physical cable pull within ~180 ms
  if (activeSocket) {
    if (activeSocket.bufferedAmount > 1500) {
      console.log(` \x1b[31m[SOCKET DRAIN STALLED]\x1b[0m: Cable pulled (${activeSocket.bufferedAmount} bytes buffered). Tripping failover...`);
      try { activeSocket.close(); } catch (_) {}
      activeSocket = null;
      if (currentMode === "USB_TETHER") {
        initConnection(WIFI_URL, "WIFI_UNTETHERED");
      } else {
        currentMode = "OFFLINE_FIELD";
        console.log(" \x1b[33m[EXPEDITION OFFLINE]\x1b[0m: Untethered field surveying active.");
      }
    }
  }
  ...
  if (superStepEpoch % 4 === 0) {
    const modeTag = currentMode === "USB_TETHER" 
      ? "\x1b[32mUSB\x1b[0m" 
      : (currentMode === "WIFI_UNTETHERED" ? "\x1b[35mWIFI\x1b[0m" : "\x1b[33mFIELD OFFLINE\x1b[0m");
